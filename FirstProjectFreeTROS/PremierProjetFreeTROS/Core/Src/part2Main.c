  ...
  for(;;)
	{
		if(flag  == 1){
			...
			//RED LED still on in phase 4 and 5 and 6
			osDelay(14000);

			//Turn off LED in phase 7
  ...
  for(;;)
	{
		...
		//Don't have to do anything in Phase 2 and 3, RED LED is still on.
		if(flag == 1){
			osDelay(4000);
			//Phase 4, RED pedestrian LED turns off
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_RESET);
			osDelay(10000);
			//Phase 5, RED pedestrian LED turns on
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_SET);
			osDelay(3000);
			flag = 0;
		}
...
for(;;)
  {
    ...
    if(flag == 1){
      // If flag is set, turn off the Green LED
